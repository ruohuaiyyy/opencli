...
cli({
  ...
  func: async (page: IPage | null, kwargs) => {
    ...
    // If category is specified, try to find and click it
    if (kwargs.category) {
    ...
    // For search pages, look for article cards with title elements
    // The actual hrefs may be constructed from data attributes
    ...
    let filtered = (Array.isArray(articles) ? articles : []).slice(0, limit);
