/**
 * Shared utilities for Yuanbao web chat automation.
 *
 * Provides: page navigation, message sending, response polling,
 * and conversation management for yuanbao.tencent.com/chat.
 */

import type { IPage } from '../../types.js';

export const YUANBAO_DOMAIN = 'yuanbao.tencent.com';
export const YUANBAO_CHAT_URL = 'https://yuanbao.tencent.com/chat';
...
export interface YuanbaoPageState {
  url: string;
  title: string;
  ...
  return `
    (() => {
      const placeholderNode = document.querySelector(
        'textarea[placeholder], [contenteditable="true"][placeholder], [aria-label*="输入"], [aria-label*="Message"]'
      );
      return {
        url: window.location.href,
        title: document.title || '',
        isLogin: !document.cookie.includes('uins=') || document.cookie.length > 100 ? true : null,
 ...
 * Uses data-testid and class-based selectors to identify message roles.
 */
function getTurnsScript(): string {
  return `
    (() => {
      ...
      const getRole = (root) => {
        // User messages: typically aligned right or have specific classes
        if (
        ...
        // Assistant messages: typically aligned left or have specific classes
        if (
...
function fillComposerScript(text: string): string {
  return `
    ((inputText) => {
      ...
      if (!composer) throw new Error('Could not find Yuanbao input element');
      ...
      throw new Error('Unsupported Yuanbao input element');
    })(${JSON.stringify(text)})
  `;
}
...
function fillAndSubmitComposerScript(text: string): string {
  return `
    ((inputText) => {
      ...
      if (!(composer instanceof HTMLTextAreaElement || composer instanceof HTMLInputElement)) {
        throw new Error('Could not find Yuanbao textarea input element');
      }
...
function normalizeYuanbaoTabs(rawTabs: unknown[]): YuanbaoTabInfo[] {
  return rawTabs
    .map((tab, index) => {
    ...
    .filter((tab) => tab.url.includes('yuanbao.tencent.com/chat'));
}

async function selectPreferredYuanbaoTab(page: IPage): Promise<boolean> {
  const rawTabs = await page.tabs().catch(() => []);
  if (!Array.isArray(rawTabs) || rawTabs.length === 0) return false;

  const tabs = normalizeYuanbaoTabs(rawTabs);
  if (tabs.length === 0) return false;

  const preferred = [...tabs].sort((left, right) => {
    const score = (tab: YuanbaoTabInfo): number => {
      let value = tab.index;
      if (/https:\/\/yuanbao\.tencent\.com\/chat\/[A-Za-z0-9_-]+/.test(tab.url)) value += 1000;
      else if (tab.url.startsWith(YUANBAO_CHAT_URL)) value += 100;
 ...
 */
export async function ensureYuanbaoChatPage(page: IPage): Promise<void> {
  ...
  const reusedTab = await selectPreferredYuanbaoTab(page);
  if (reusedTab) {
 ...
 */
export async function getYuanbaoPageState(page: IPage): Promise<YuanbaoPageState> {
  await ensureYuanbaoChatPage(page);
  return await page.evaluate(getStateScript()) as YuanbaoPageState;
}

/**
 * Get visible conversation turns (User/Assistant messages).
 */
export async function getYuanbaoTurns(page: IPage): Promise<YuanbaoTurn[]> {
  await ensureYuanbaoChatPage(page);
  const turns = await page.evaluate(getTurnsScript()) as YuanbaoTurn[];
 ...
 */
export async function getYuanbaoVisibleTurns(page: IPage): Promise<YuanbaoTurn[]> {
  await ensureYuanbaoChatPage(page);
  return await page.evaluate(getTurnsScript()) as YuanbaoTurn[];
}

/**
 * Get transcript lines (full page text extraction).
 */
export async function getYuanbaoTranscriptLines(page: IPage): Promise<string[]> {
  await ensureYuanbaoChatPage(page);
 ...
 */
export async function sendYuanbaoMessage(page: IPage, text: string): Promise<'button' | 'enter'> {
  await ensureYuanbaoChatPage(page);
 ...
 */
export async function waitForYuanbaoResponse(
  page: IPage,
  beforeLines: string[],
  beforeTurns: YuanbaoTurn[],
  promptText: string,
  ...
  const getCandidate = async (): Promise<string> => {
    ...
    const lines = await getYuanbaoTranscriptLines(page);
...
export async function startNewYuanbaoChat(page: IPage): Promise<string> {
  ...
  await page.goto(YUANBAO_CHAT_URL, { waitUntil: 'load', settleMs: 2000 });
  await page.wait(1.5);
  return 'navigate';
}
