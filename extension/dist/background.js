...
*/
async function dispatchMouseEvent(tabId, type, x, y, button = "left") {
	await ensureAttached(tabId);
...
async function handleCommand(cmd) {
	...
	try {
		switch (cmd.action) {
			...
			case "mouse": return await handleMouse(cmd, workspace);
			default: return {
				id: cmd.id,
				ok: false,
...
async function resolveTabId(tabId, workspace) {
	if (tabId !== void 0) try {
		...
		if (session && tab.windowId !== session.windowId) console.warn(`[opencli] Tab ${tabId} belongs to window ${tab.windowId}, not automation window ${session.windowId}, re-resolving`);
...
async function handleMouse(cmd, workspace) {
	const tabId = await resolveTabId(cmd.tabId, workspace);
	try {
		await dispatchMouseEvent(tabId, cmd.mouseType || "mouseMoved", cmd.x ?? 0, cmd.y ?? 0, cmd.button || "left");
		return {
			id: cmd.id,
			ok: true,
			data: "mouse event dispatched"
		};
	} catch (err) {
		return {
			id: cmd.id,
